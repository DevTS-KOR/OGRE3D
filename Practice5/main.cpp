...
float fDeahyunMoveZ = 0.0f;
int fDeahyunMoveZPlus = 1.0f;
float fFishAngle = 0.0f;
float fDgree = 1.0f;
int iRotateCount = 0;
...
public:
  ...
  bool frameStarted(const FrameEvent &evt)
  {
	  ...
	  mProfessorNode->setPosition(0, 0, fDeahyunMoveZ);
	  fDeahyunMoveZ += fDeahyunMoveZPlus;

	  if (fDeahyunMoveZ <= -250)
	  {

		  mProfessorNode->yaw(Degree(fDgree));
	  ...
	  mfishNode->setPosition(cos(fFishAngle * (3.141592 / 180.0f)) * 100.0f, 0.0f, sin(fFishAngle * (3.141592 / 180.0f)) * 100.0f);
	  fFishAngle += 0.5f;
