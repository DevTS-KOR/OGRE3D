...
int CQueue::GetSize()
{
	return (m_iHead - m_iTail + BUFF_SIZE) & BUFF_MASK;
...
//		AfxMessageBox("Error while creating ThreadWatchComm, " + pComm->m_sPortName);
	...
	while (pComm->m_bConnected)
	{
		...
		if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
		{
			...
			do	
			{
				dwRead = pComm->ReadComm( tmpBuff, 2048);
				
				if (BUFF_SIZE - pComm->m_QueueRead.GetSize() > (int)dwRead)
				{		// 큐에 여유가 있으면 읽은 것을 큐에 넣어준다.
					for ( WORD i = 0; i < dwRead; i++)
						pComm->m_QueueRead.PutByte(tmpBuff[i]);
				}
				else	// 여유가 없으면 에러 메시지 ! (읽은 data는 손실됨)
					dwRead = dwRead;
//					AfxMessageBox("m_QueueRead FULL!");
			  
//##$$			} while (dwRead && pComm->m_bConnected);	// 또 있으면 또 읽자 ~ !
			} while (dwRead);	// 또 있으면 또 읽자 ~ !
			//	읽어 가도록 메시지를 보낸다.
//		::PostMessage(*h_pCommWnd, WM_COMM_READ, pComm->m_wPortID, 0);
		// 이제 메시지를 보낸다 ~ !
		}
	}	
